...
function Testimonials() {
    ...
    const [Testinomials, setTestinoMials] = useState([]);
    useEffect(() => {
        axios.get(`${baseUrl()}/homePageData/Testinomials`,{}, {
            headers: {
                    ...
                    <Carousel responsive={responsive} >
                        {Testinomials?.map((item) =>{
                            ...
                            <Carousel.Item className="">
                                <h3 className="sec_caro_h3 " >Success Stories & Tesimonials</h3>
                                <Row className=" bg_radious">
                                    <Col md={6} sm={12} xs={12}>
                                        <Image src="./assets/testimonials/testimonial.png" alt='frame' className="Caor_img" fluid />
                                    </Col>
                                    <Col md={6} sm={12} xs={12} className="sec_caro_part margin-l-15px border-right-radius-20px">
                                        <h3 className="Caro_head">{item.name}</h3>
                                        <p className="Caor_para">{item.content}</p>
                                        <button type="button" className="caro_btn">View Full Story <Image src="./assets/caroselimg/Frame 185460.png " alt="icon" />
                                        </button>
                                    </Col>
                                </Row>
                            </Carousel.Item>
